...
export class SessionsChatAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor) {
		...
		content.push(localize('sessionsChat.nextUserMessageSuggestion', "When a suggested next message is available, press {0} to accept it. Typing or pressing {1} dismisses it.", '<keybinding:editor.action.inlineSuggest.commit>', '<keybinding:editor.action.inlineSuggest.hide>'));
		...
		content.push(localize('sessionsChat.feedbackComments', "When pull requests have failing checks or unreviewed comments, one banner appears above the input. If several pull requests need attention, use the Previous Banner and Next Banner buttons to move between them. A pull request with both failing checks and comments uses split buttons: activate the main action to address both, or use its More Actions button to address or reveal only CI or comments. In-product agent review comments appear as their own carousel item."));
		...
		content.push(localize('sessionsChat.failingChecksPullRequest', "When a session pull request has failing checks, use Reveal CI in its banner item to open that pull request, or use Fix CI to ask the agent to address the failures."));
